...
- 可选在录音保存后自动调用 ASR 转录，并将文本推送到企业微信群机器人；支持仅处理自动接听录音或所有通话录音。

在“设置 → 自动应答”中设置自动接听延时，并在该页“录音转录与推送”中配置 ASR 服务地址（默认
`http://DELLXPS.local:9100`）、API Key、语言及企业微信 Webhook，然后开启自动处理。
此功能默认关闭，与短信转发开关独立；可复制现有短信转发的企业微信地址。
新增配置和密钥直接保存在本机 `~/Library/Application Support/CellDock/CallTranscriptions/settings.json`
中，不使用钥匙串，文件仅限当前用户读写。USB 音频输入同样需要系统麦克风权限；
自动接听的仅听模式不会采集 Mac 麦克风。

该页的“接通欢迎语”可配置欢迎语文字、独立的 TTS API Key、音色、语速和服务地址
（默认 `http://DELLXPS.local:9101`）。点击“生成并保存”会通过 `POST /v1/audio/speech`
生成语音并转换为电话使用的 8 kHz PCM；保存后可“试听”。文本最多 5000 字，语音最长 5 分钟。
开启后，仅在自动接通且音频就绪时向来电方播放一次，挂断会停止。合成失败保留原配置；
新配置从下次自动接听生效。缓存使接听时无需等待 TTS 服务，Mac 麦克风仍不采集。
配置、密钥和缓存一起保存于 `~/Library/Application Support/CellDock/CallWelcome/welcome.json`
（仅当前用户读写，不使用钥匙串）。欢迎语使用录音的上行声道；自动接听录音的 ASR
只处理来电方声道，因此不会把欢迎语主动加入转录。

录音会转换为单声道 WAV 并分成最长 5 分钟的片段，原录音不变。ASR 使用
`POST /v1/audio/transcriptions`、`X-API-Key` 和 `qwen3-asr-0.6b`，服务忙时有限次重试。
长文本按企业微信 2048 字节上限分条推送。录音详情可查看本地转录结果、状态和重试按钮；
推送失败后重试会复用已有文本；如果更换 Webhook，会向新地址发送完整文本。
关闭开关会暂停未完成任务，重新开启或重启后继续；
已发送的消息不会因关闭开关或删除本地录音而撤回。网络中断时若服务端已接收但确认未送达，
重试可能重复该条消息。Mac 需保持运行并能访问 ASR 服务和企业微信。
