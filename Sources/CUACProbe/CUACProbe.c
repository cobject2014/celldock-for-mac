...
#define CELLDOCK_UAC_TARGET_RATE 8000.0
#define CELLDOCK_UAC_TEXT_CAPACITY 512U
#define CELLDOCK_UAC_ERROR_CAPACITY 4096U
#define CELLDOCK_UAC_SIGNAL_THRESHOLD_PCM16 256U
#define CELLDOCK_UAC_PCM_RING_FRAMES 2048U
#define CELLDOCK_UAC_CONVERSION_CHUNK_FRAMES 256U

typedef enum {
    CELLDOCK_UAC_SAMPLES_UNSUPPORTED = 0,
    CELLDOCK_UAC_SAMPLES_FLOAT32 = 1,
    CELLDOCK_UAC_SAMPLES_INT16_NATIVE = 2,
    CELLDOCK_UAC_SAMPLES_INT16_SWAPPED = 3
} UACSampleKind;

typedef struct {
    int16_t frames[CELLDOCK_UAC_PCM_RING_FRAMES];
...
typedef struct {
    ...
    char name[CELLDOCK_UAC_TEXT_CAPACITY];
    char uid[CELLDOCK_UAC_TEXT_CAPACITY];
...
struct CellDockUACProbe {
    UACEndpoint input;
    UACEndpoint output;
    char name[CELLDOCK_UAC_TEXT_CAPACITY];
    char uid[CELLDOCK_UAC_TEXT_CAPACITY];
    char last_error[CELLDOCK_UAC_ERROR_CAPACITY];
    double sample_rate;
...
static void clear_error(CellDockUACProbe *probe) {
    if (probe != NULL) {
        probe->last_error[0] = '\0';
    }
}

static void set_error(CellDockUACProbe *probe, const char *format, ...) {
    if (probe == NULL) {
        return;
    }
    va_list arguments;
...
static UACSampleKind sample_kind_for_format(
    ...
    if (format == NULL || format->mFormatID != kAudioFormatLinearPCM ||
        ...
        return CELLDOCK_UAC_SAMPLES_UNSUPPORTED;
    }
    if ((format->mFormatFlags & kAudioFormatFlagIsFloat) != 0 &&
        (format->mFormatFlags & kAudioFormatFlagIsBigEndian) == 0 &&
        format->mBitsPerChannel == 32 &&
        format->mBytesPerFrame == sizeof(float)) {
        return CELLDOCK_UAC_SAMPLES_FLOAT32;
    }
    if ((format->mFormatFlags & kAudioFormatFlagIsSignedInteger) != 0 &&
        ...
        return (format->mFormatFlags & kAudioFormatFlagIsBigEndian) == 0
            ? CELLDOCK_UAC_SAMPLES_INT16_NATIVE
            : CELLDOCK_UAC_SAMPLES_INT16_SWAPPED;
    }
    return CELLDOCK_UAC_SAMPLES_UNSUPPORTED;
}
...
static size_t pcm_ring_write(
    ...
    if (used > CELLDOCK_UAC_PCM_RING_FRAMES) {
        used = CELLDOCK_UAC_PCM_RING_FRAMES;
    }
    size_t available = (size_t)(CELLDOCK_UAC_PCM_RING_FRAMES - used);
    size_t accepted = frame_count < available ? frame_count : available;
    for (size_t index = 0; index < accepted; index++) {
        ring->frames[(write_index + index) % CELLDOCK_UAC_PCM_RING_FRAMES] = frames[index];
    }
...
static size_t pcm_ring_read(
    ...
    if (available64 > CELLDOCK_UAC_PCM_RING_FRAMES) {
        available64 = CELLDOCK_UAC_PCM_RING_FRAMES;
    }
    size_t available = (size_t)available64;
    size_t copied = maximum_frames < available ? maximum_frames : available;
    for (size_t index = 0; index < copied; index++) {
        frames[index] = ring->frames[(read_index + index) % CELLDOCK_UAC_PCM_RING_FRAMES];
    }
    atomic_store_explicit(
        &ring->read_index,
        read_index + copied,
...
static void update_input_peak(CellDockUACProbe *probe, uint32_t candidate) {
...
static int16_t pcm16_sample_from_bytes(
    ...
    if (kind == CELLDOCK_UAC_SAMPLES_FLOAT32) {
        float sample = 0;
        memcpy(&sample, bytes, sizeof(sample));
        if (!isfinite(sample)) {
            return 0;
    ...
    uint16_t bits = 0;
    memcpy(&bits, bytes, sizeof(bits));
    if (kind == CELLDOCK_UAC_SAMPLES_INT16_SWAPPED) {
        bits = (uint16_t)((bits >> 8) | (bits << 8));
    }
    int16_t sample = 0;
    memcpy(&sample, &bits, sizeof(sample));
...
static void enqueue_input_pcm(
    ...
    if (probe == NULL || !mono_buffer_list_is_valid(input_data) ||
        probe->input.sample_kind == CELLDOCK_UAC_SAMPLES_UNSUPPORTED) {
        return;
    }
    size_t sample_size = probe->input.sample_kind == CELLDOCK_UAC_SAMPLES_FLOAT32
        ? sizeof(float)
        : sizeof(int16_t);
    int16_t converted[CELLDOCK_UAC_CONVERSION_CHUNK_FRAMES];
    for (UInt32 buffer_index = 0;
        ...
        while (remaining > 0) {
            size_t chunk = remaining < CELLDOCK_UAC_CONVERSION_CHUNK_FRAMES
                ? remaining
                : CELLDOCK_UAC_CONVERSION_CHUNK_FRAMES;
...
static void write_output_pcm(
    ...
    for (UInt32 buffer_index = 0;
        ...
        if (probe->output.sample_kind == CELLDOCK_UAC_SAMPLES_UNSUPPORTED ||
        ...
        while (frame_count > 0) {
            size_t chunk = frame_count < CELLDOCK_UAC_CONVERSION_CHUNK_FRAMES
                ? frame_count
                : CELLDOCK_UAC_CONVERSION_CHUNK_FRAMES;
            ...
            for (size_t index = 0; index < chunk; index++) {
                ...
                if (probe->output.sample_kind == CELLDOCK_UAC_SAMPLES_FLOAT32) {
                    float value = (float)sample / 32768.0f;
                    memcpy(frame, &value, sizeof(value));
                } else {
                    uint16_t bits = 0;
                    memcpy(&bits, &sample, sizeof(bits));
                    if (probe->output.sample_kind == CELLDOCK_UAC_SAMPLES_INT16_SWAPPED) {
                        bits = (uint16_t)((bits >> 8) | (bits << 8));
                    }
...
static void accumulate_input_signal(
    ...
    for (UInt32 buffer_index = 0;
        ...
        for (size_t sample_index = 0; sample_index < count; sample_index++) {
            ...
            if (probe->input.sample_kind == CELLDOCK_UAC_SAMPLES_FLOAT32) {
                magnitude = float32_pcm16_magnitude(sample_bytes);
            } else {
                magnitude = int16_pcm16_magnitude(
                    sample_bytes,
                    probe->input.sample_kind == CELLDOCK_UAC_SAMPLES_INT16_SWAPPED
                );
            }
            if (magnitude > peak) {
                peak = magnitude;
            }
            if (magnitude > CELLDOCK_UAC_SIGNAL_THRESHOLD_PCM16) {
                signal_samples++;
            }
        }
        total_samples += count;
...
static OSStatus audio_io_proc(
    ...
    (void)input_time;
    (void)output_time;
    CellDockUACProbe *probe = context;
    if (probe == NULL) {
        return noErr;
    }
    atomic_fetch_add_explicit(
...
CellDockUACProbe *celldock_uac_probe_create(void) {
    CellDockUACProbe *probe = calloc(1, sizeof(CellDockUACProbe));
    if (probe != NULL) {
...
static int restore_endpoint_sample_rate(
    ...
    return CELLDOCK_UAC_UNSUPPORTED;
}
...
static int probe_has_unresolved_io(const CellDockUACProbe *probe) {
    return probe != NULL &&
...
int celldock_uac_probe_stop(CellDockUACProbe *probe) {
    ...
    char errors[CELLDOCK_UAC_ERROR_CAPACITY] = {0};
    ...
    if (probe->output.device_id != probe->input.device_id) {
        ...
        if (first_result == CELLDOCK_UAC_OK && output_result != CELLDOCK_UAC_OK) {
    ...
    if (first_result != CELLDOCK_UAC_OK) {
...
void celldock_uac_probe_close(CellDockUACProbe *probe) {
    ...
    (void)celldock_uac_probe_stop(probe);
...
int celldock_uac_probe_try_destroy(CellDockUACProbe *probe) {
    ...
    celldock_uac_probe_close(probe);
    if (probe_has_unresolved_io(probe)) {
...
void celldock_uac_probe_destroy(CellDockUACProbe *probe) {
    ...
    (void)celldock_uac_probe_try_destroy(probe);
}
...
int celldock_uac_probe_open_for_usb(
    ...
    if (status != noErr || size < sizeof(AudioObjectID)) {
        ...
        return status == noErr ? CELLDOCK_UAC_NOT_FOUND : (int)status;
    }
    ...
    char inventory[CELLDOCK_UAC_ERROR_CAPACITY] = {0};
    ...
    for (size_t index = 0; index < count; index++) {
        ...
        char name[CELLDOCK_UAC_TEXT_CAPACITY] = {0};
        char uid[CELLDOCK_UAC_TEXT_CAPACITY] = {0};
        ...
        int supports_target_rate = available_rate_includes(device, CELLDOCK_UAC_TARGET_RATE);
        append_text(
            inventory,
            sizeof(inventory),
    ...
    for (size_t input_index = 0; input_index < candidate_count; input_index++) {
        ...
        for (size_t output_index = 0; output_index < candidate_count; output_index++) {
            ...
            if (explicit_uid) {
                char pair_uid[CELLDOCK_UAC_TEXT_CAPACITY] = {0};
    ...
    if (probe->input.sample_kind == CELLDOCK_UAC_SAMPLES_UNSUPPORTED) {
...
static int set_endpoint_target_sample_rate(
    ...
    if (fabs(current - CELLDOCK_UAC_TARGET_RATE) < 0.5) {
        return CELLDOCK_UAC_OK;
    }
    if (!available_rate_includes(endpoint->device_id, CELLDOCK_UAC_TARGET_RATE)) {
        append_text(
            errors,
            errors_capacity,
    ...
    if (status != noErr || !settable) {
        ...
        return status == noErr ? CELLDOCK_UAC_UNSUPPORTED : (int)status;
    }
    double target = CELLDOCK_UAC_TARGET_RATE;
...
static int fail_start_and_cleanup(
    ...
    int cleanup_result = celldock_uac_probe_stop(probe);
    if (cleanup_result == CELLDOCK_UAC_OK) {
        set_error(probe, "%s", start_error);
    } else {
        char cleanup_error[CELLDOCK_UAC_ERROR_CAPACITY] = {0};
...
static void refresh_endpoint_format(
    ...
    endpoint->sample_kind = CELLDOCK_UAC_SAMPLES_UNSUPPORTED;
...
static int endpoint_format_is_bridge_ready(const UACEndpoint *endpoint) {
    if (endpoint == NULL || endpoint->sample_kind == CELLDOCK_UAC_SAMPLES_UNSUPPORTED) {
        return 0;
    }
    const AudioStreamBasicDescription *format = &endpoint->virtual_format;
    return fabs(format->mSampleRate - CELLDOCK_UAC_TARGET_RATE) < 0.5 &&
        format->mChannelsPerFrame == 1 &&
...
int celldock_uac_probe_start_pcm_bridge(CellDockUACProbe *probe) {
    ...
    clear_error(probe);
    char start_error[CELLDOCK_UAC_ERROR_CAPACITY] = {0};
    ...
    if (rate_result != CELLDOCK_UAC_OK) {
    ...
    if (!input_format_ready) {
        ...
        return fail_start_and_cleanup(probe, CELLDOCK_UAC_UNSUPPORTED, start_error);
    }
...
int celldock_uac_probe_start_silence(CellDockUACProbe *probe) {
    return celldock_uac_probe_start_pcm_bridge(probe);
}
...
void celldock_uac_probe_flush_pcm(CellDockUACProbe *probe) {
    celldock_uac_probe_flush_downlink_pcm(probe);
...
static int endpoint_original_device_is_alive(const UACEndpoint *endpoint) {
    ...
    char current_uid[CELLDOCK_UAC_TEXT_CAPACITY] = {0};
    if (!string_property(
        endpoint->device_id,
...
uint32_t celldock_uac_probe_input_signal_threshold_pcm16(const CellDockUACProbe *probe) {
    (void)probe;
    return CELLDOCK_UAC_SIGNAL_THRESHOLD_PCM16;
}
